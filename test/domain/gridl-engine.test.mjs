...
const gridlRoot = repositoryRoot;
const stateEngine = await import("../../src/games/gridl/engine/state.mjs");
const rulesEngine = await import("../../src/games/gridl/engine/rules.mjs");
const levelEngine = await import("../../src/games/gridl/engine/level-loader.mjs");
...
test("Gridl golden level 101 completes CAT at par", () => {
  ...
  const raw = JSON.parse(
    fs.readFileSync(
      path.join(
        gridlRoot,
        `public/gridl/levels/level-${trace.levelId}.json`,
      ),
      "utf8",
    ),
  );
...
test("Gridl normalizes every level and identifies placeholder content", () => {
  const levelsDirectory = path.join(gridlRoot, "public/gridl/levels");
  const files = fs
...
test("Gridl preserves a portal cell's special state when moving a staged tile", () => {
    const { state } = startCustomLevel({
      rows: 2,
      cols: 2,
      goal: { r: 1, c: 1 },
