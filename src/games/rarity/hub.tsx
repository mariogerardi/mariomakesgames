...
export function RarityHubMark() {
  return <Image alt="" className="rarity-collection-gem" decoding="sync" height={160} priority src="/hub/rarity-gem.png" unoptimized width={160} />;
}
...
function PreviewRarityWord({ word }: { word: string }) {
  const matchAt = word.toLowerCase().indexOf("wel");
...
export function RarityHubPreview({ instance, phase, registerCard, typedAnswer }: HubPreviewProps) {
  ...
  return (
    <div
      className={`preview-card preview-card-rarity is-${phase}`}
      data-preview-game="rarity"
      data-preview-phase={phase}
      ref={cardRef}
      style={{ "--preview-rarity-fill": `${score}%` } as CSSProperties}
    >
      <div className="preview-rarity-brand"><Image alt="" decoding="sync" height={160} priority src="/hub/rarity-gem.png" unoptimized width={160} /><span>rarity</span></div>
      <div className="preview-rarity-panel">
        {phase === "feedback" ? (
          <div className="preview-rarity-result">
            ...
            <div className="preview-rarity-result-tier"><b>rare</b><span>rare territory. an impressive pull.</span></div>
            <div className="preview-rarity-tier-track" aria-hidden="true">
              <i className="is-earned" /><i className="is-earned" /><i className="is-earned" /><i className="is-earned is-current" /><i /><i />
            </div>
          </div>
        ) : <>
          <div className="preview-rarity-string-block"><small>today’s string</small><strong>WEL</strong><p>one valid word. make it as rare as you can.</p></div>
          <div className="preview-rarity-entry">
            <span className="preview-entry-value" data-preview-entry>{typedAnswer ? <PreviewRarityWord word={typedAnswer} /> : "\u00a0"}</span>
          </div>
        </>}
      </div>
    </div>
