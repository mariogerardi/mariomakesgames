...
export const DUAL_DAILY_EPOCH = CURRENT_DAILY_EPOCH;
...
export function selectDailyDualPuzzle(date = new Date()) {
  const index = dailyCatalogIndex(date, dualPuzzles.length, DUAL_DAILY_EPOCH);
...
export function dualArchive(days = 14, today = new Date()) {
  return Array.from({ length: days }, (_, index) => {
    ...
    return dailyCatalogOffset(date, DUAL_DAILY_EPOCH) < 0 ? null : {
      date,
